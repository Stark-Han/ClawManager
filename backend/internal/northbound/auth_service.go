...
func NewAuthService(repo AuthRepository, users repository.UserRepository, cfg config.NorthboundConfig, decryptor *JWEDecryptor) (*AuthService, error) {
...
func (s *AuthService) Login(ctx context.Context, challengeID, compactJWE, sourceIP, userAgent string) (*TokenResponse, error) {
	...
	if !s.accountLimiter.Allow(accountKey) {
	...
	if issuedAt.After(now.Add(30*time.Second)) || now.Sub(issuedAt) > s.config.ChallengeTTL {
		return nil, apiError(401, "INVALID_CREDENTIALS", "Invalid username or password", nil)
	}
...
func (s *AuthService) createSession(userID int, sourceIP, userAgent string, scopes []string) (*TokenResponse, error) {
	...
	access, accessExpiry, err := issueAccessToken(s.config.JWTSecret, principal, s.config.AccessTokenTTL)
	if err != nil {
		return nil, err
	}
	ip := strings.TrimSpace(sourceIP)
