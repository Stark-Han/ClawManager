...
OpenCode Lite 会在实例自己的持久化工作区中自动创建空的 `starter` 文件夹，并将它作为默认启动目录。控制台直接进入该目录的新会话路由，因此打开实例时已经选中 `starter`，可以立即开始一个空白对话。这个过程不显示额外的工作区选项或新手向导；重启和 Runtime 滚动升级也不会覆盖其中的用户文件。
